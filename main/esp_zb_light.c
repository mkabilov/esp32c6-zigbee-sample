...
#define GPIO_LED_BLUE   GPIO_NUM_23
    ...
    if (message->info.dst_endpoint == HA_ESP_LIGHT_ENDPOINT) {
        if (message->info.cluster == ESP_ZB_ZCL_CLUSTER_ID_ON_OFF) {
            if (message->attribute.id == ESP_ZB_ZCL_ATTR_ON_OFF_ON_OFF_ID &&
                ...
                if (light_state) {
                    led_red();
                    ESP_LOGI(TAG, "Light sets to on");
                } else {
                    led_off();
                    ESP_LOGI(TAG, "Light sets to off");
                }
            }
        }
    }
...
static void esp_zb_task(void *pvParameters) {
    ...
    esp_zb_ep_list_add_ep(esp_zb_ep_list, esp_zb_cluster_list, HA_ESP_LIGHT_ENDPOINT, ESP_ZB_AF_HA_PROFILE_ID,
